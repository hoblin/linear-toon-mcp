...
| `save_project_milestone` | Create or update a project milestone. On create, `name` and `project` are required. `sortOrder` positions the milestone among its siblings. Milestones cannot be moved between projects, so `project` is rejected on update. |
