...
| `save_issue` | Create or update a Linear issue (id presence determines). Accepts human-friendly names for team, assignee, state, labels, project, cycle, and milestone. Relation params (`blocks`, `relatedTo`, `duplicateOf`) and `parentId` accept either issue UUIDs or human identifiers (e.g., `LIN-123`). On create, relations are appended; on update, they replace existing. Null values remove fields (update only). |
| `save_comment` | Create or update a comment (id presence determines). On create, exactly one of `issue`, `project`, `initiative`, or `projectUpdate` identifies the parent (name or UUID where applicable). Supports threaded replies via `parentId`. |
| `delete_comment` | Delete a comment by ID. |
| `list_comments` | List comments on an issue, project, initiative, or project status update. Exactly one parent (name or UUID where applicable) is required. Cursor-paginated. |
