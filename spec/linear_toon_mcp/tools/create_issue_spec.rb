...
RSpec.describe LinearToonMcp::Tools::CreateIssue do
  describe ".call" do
    ...
    before do
      allow(LinearToonMcp::Resolvers::TeamResolver).to receive(:call).with(client, value: team_id).and_return(team_id)
    ...
    context "with resolved fields" do
      ...
      before do
        allow(LinearToonMcp::Resolvers::TeamResolver).to receive(:call).with(client, value: "Engineering").and_return(team_id)
        allow(LinearToonMcp::Resolvers::UserResolver).to receive(:call).with(client, value: "Alice").and_return("user-uuid")
        allow(LinearToonMcp::Resolvers::WorkflowStateResolver).to receive(:call).with(client, value: "In Progress", team_id: team_id).and_return("state-uuid")
        allow(LinearToonMcp::Resolvers::IssueLabelResolver).to receive(:call_many).with(client, values: ["bug", "urgent"], team_id: team_id).and_return(["l1", "l2"])
        allow(LinearToonMcp::Resolvers::ProjectResolver).to receive(:call).with(client, value: "My Project").and_return("proj-uuid")
        allow(LinearToonMcp::Resolvers::CycleResolver).to receive(:call).with(client, value: "Sprint 5", team_id: team_id).and_return("cycle-uuid")
      end
    ...
    context "with milestone" do
      ...
      before do
        allow(LinearToonMcp::Resolvers::ProjectResolver).to receive(:call).with(client, value: "My Project").and_return("proj-uuid")
        allow(LinearToonMcp::Resolvers::ProjectMilestoneResolver).to receive(:call).with(client, value: "MVP", project_id: "proj-uuid").and_return("ms-uuid")
      end
