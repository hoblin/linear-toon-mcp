...
RSpec.describe LinearToonMcp::Tools::ListIssueLabels do
  describe ".call" do
    ...
    it "queries all labels without filter" do
      response
    ...
    context "with team filter" do
      ...
      it "resolves the team and queries with team filter" do
        response
        expect(LinearToonMcp::Resolvers::TeamResolver).to have_received(:call).with(client, value: "Engineering")
    ...
    context "with team UUID" do
      ...
      it "passes UUID through the resolver" do
        response
        expect(LinearToonMcp::Resolvers::TeamResolver).to have_received(:call).with(client, value: team_id)
      end
    end

    context "when labels list is empty" do
      before do
    ...
    context "when issueLabels field is nil" do
      before do
        allow(client).to receive(:query).and_return("issueLabels" => nil)
      end
